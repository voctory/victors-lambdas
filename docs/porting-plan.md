# Victor's Lambdas Porting Plan

This document tracks implementation status for Victor's Lambdas, an unofficial Rust toolkit for AWS Lambda
...
- Crates: one umbrella crate, `victors-lambdas`, plus utility crates under `crates/`.
...
| Logger | `LoggerConfig`, `LogLevel`, `Logger`, `LogEntry`, `LogValue`, `LogBufferConfig`, `LogBuffer`, `LogBufferError`, `LogFormatter`, `LogRedactor`, `JsonLogFormatter`, `LambdaContextFields`, `LoggerLayer`, JSON rendering, `POWERTOOLS_DEV` pretty rendering, persistent fields, temporary fields, event rendering toggle, level filtering, debug sampling, correlation ID helpers, Lambda context fields, key redaction, custom formatter/redaction hook APIs, bounded request-keyed log buffering, optional `tracing` subscriber integration, stdout emission, and initial docs/snippet | Broader handler examples |
| Metrics | `MetricsConfig`, `Metric`, `MetricUnit`, `MetricResolution`, `MetadataValue`, EMF JSON renderer, request dimensions, default dimensions, metadata, name/value validation, service dimension, cold-start metric, high-resolution metric definitions, stdout flush API, explicit timestamp rendering/writing, opt-in overflow flush helpers, async capture helpers, `POWERTOOLS_DEV` disable behavior, CloudWatch limits, and initial docs/snippet | Broader handler examples |
...
| Parameters | `ParameterProvider`, `AsyncParameterProvider`, `Parameters`, `AsyncParameters`, `Parameter`, `ParameterTransform`, `ParameterValue`, `CachePolicy`, async provider/retrieval errors, in-memory provider, optional SSM single-parameter, by-name, and path providers with decryption plus set operations, optional Secrets Manager, AppConfig, and DynamoDB providers, force-fetch support, JSON transforms, base64 binary transforms, suffix-based auto transforms, initial docs/snippet, and buildable AWS provider snippet | Additional deployment examples |
| JMESPath | `JmespathExpression`, `search`, `search_as`, `query`, `extract_data_from_envelope`, Powertools `powertools_json`, `powertools_base64`, and `powertools_base64_gzip` functions, common Lambda event envelope constants, optional idempotency key extraction and payload validation integration, optional validation envelope integration, and initial docs/snippet | Data-masking integration examples |
| Data masking | `DataMasking`, whole-value erasure, field erasure by JSON Pointer, dot path, or common JSONPath-style selectors, fixed/dynamic/custom/regex masking strategies, per-field masking rules, provider encryption/decryption, optional direct AWS KMS provider, configurable missing-field behavior, initial docs/snippet, and umbrella exports | AWS Encryption SDK envelope encryption/cached data keys and advanced JSONPath operators beyond the supported selector subset |
| Kafka | `KafkaConsumer`, `KafkaSchemaConsumer`, `ConsumerRecords`, `ConsumerRecord`, primitive and `JSON` base64 key/value deserializers, schema-aware key/value decoder configuration with Event Source Mapping metadata capture, optional Avro and Protobuf base64 decode helpers, decoded `UTF-8` headers, original encoded field/header preservation, initial docs/snippet, and umbrella exports | Richer schema examples |
| Streaming | `RangeSource`, `SeekableStream`, `AsyncRangeSource`, `AsyncSeekableStream`, `BytesRangeSource`, `S3Object`, `S3RangeSource`/`S3ObjectClient`/`AsyncS3ObjectClient`, optional AWS SDK-backed S3 client adapter, gzip decoder, CSV reader, and ZIP archive transforms, initial docs/snippet, and umbrella exports | Larger streaming examples |
| Parser | `EventParser`, `ParsedEvent`, `ParseError`, serde JSON string/slice/value parsing, Transfer Family authorizer event/response models, AppSync Events model and publish payload envelope, Bedrock Agent OpenAPI event model and input text envelope, Bedrock Agent function-details model and input text envelope, CloudWatch dashboard custom widget model, DynamoDB stream on-failure destination model, S3 EventBridge notification model, S3 event notification model with Intelligent-Tiering support, IoT Core registry event models, Cognito migrate-user and custom sender event models, optional `aws_lambda_events` API Gateway Lambda authorizer aliases, API Gateway WebSocket lifecycle aliases, AppSync Lambda authorizer aliases, Cognito User Pool trigger aliases, common Powertools parser event and record model aliases, including Kinesis-delivered DynamoDB stream record aliases, API Gateway REST/HTTP/WebSocket API body, AppSync direct resolver/batch aliases and arguments/source, Bedrock Agent OpenAPI input text, ActiveMQ message data, ALB target group body, Lambda Function URL body, VPC Lattice v1/v2 body, EventBridge detail with Scheduler empty-detail compatibility, CloudFormation custom resource request/response aliases and properties, Cognito User Pool user attributes, SQS body, SNS message, SNS-over-SQS message, RabbitMQ message data, S3 record, owned S3 event notification record, S3-over-SQS record, owned S3-over-SQS notification record, S3 Object Lambda configuration payload, S3 Batch job task, SES record, Auto Scaling, AWS Config, CloudWatch Alarm, CodeCommit, CodeDeploy lifecycle hook, CodePipeline job, and Connect contact flow source event aliases, CloudWatch Logs message, Kinesis record data, Kinesis-delivered CloudWatch Logs messages, Kinesis-delivered DynamoDB stream image, Firehose record data, Firehose-delivered SQS body, DynamoDB stream image pairs and image-specific helpers, Kafka record value envelopes, Secrets Manager rotation source event alias, initial docs/snippet, and JSON fixtures for API Gateway REST/HTTP/WebSocket lifecycle/authorizer, VPC Lattice, AppSync, AppSync authorizer, AppSync batch, AppSync Events, Bedrock, Cognito, EventBridge, EventBridge Scheduler, SQS, ActiveMQ, ALB, Lambda Function URL, SNS, RabbitMQ, S3, S3 Intelligent-Tiering, S3 Object Lambda, S3 Batch, S3-over-SQS, SNS-over-SQS, SES, Auto Scaling, AWS Config, CloudWatch Alarm, CloudWatch custom widget, CodeCommit, CodeDeploy lifecycle hook, CodePipeline job, Connect contact flow, Secrets Manager rotation, CloudFormation request/response/stream/nested-stream events, Transfer Family, Cognito custom and migrate-user, DynamoDB stream on-failure, S3 EventBridge, and IoT registry events | Broader `aws_lambda_events` envelopes, Powertools adapters, shared event fixtures, schema-aware parsing |
| Batch | `BatchRecord`, `BatchProcessor`, `BatchProcessingReport`, `BatchRecordResult`, `BatchItemFailure`, `BatchResponse`, sequential and concurrent generic processing, stream checkpoint helpers, optional `aws_lambda_events` SQS, Kinesis, DynamoDB stream, and Kafka adapters including Kafka partition/offset partial response shape, SQS FIFO early-stop behavior, optional parser-integrated SQS message body, Kinesis record data, and DynamoDB stream image processing, and initial docs/snippet | Larger examples |
| Validation | `Validator`, `Validate`, `ValidationError`, required text, length, range, custom predicate helpers, inbound/outbound validation wrappers, optional local JSON Schema backend, compiled schema cache, optional JMESPath envelope extraction before schema validation, event-handler validation hooks, and initial docs/snippet | Richer handler examples |
| Idempotency | `IdempotencyConfig`, `PayloadValidation`, `IdempotencyKey`, `Idempotency`, `AsyncIdempotency`, `IdempotencyOutcome`, typed workflow errors, SHA-256 JSON payload hashing, JSON Pointer and optional JMESPath key extraction, configurable full/subset/no payload validation including optional JMESPath validation selection, sync and async handler wrappers, Lambda remaining-time in-progress expiry, result replay, sync and async store traits/errors/results, in-memory store, generic async external TTL cache store, local cache wrapper, optional DynamoDB store, initial docs/snippet, and buildable DynamoDB-backed snippet | Additional deployment examples |
...
| Event handler | `Method`, method parsing/matching, `Request`, `Response`, `HttpError`, `PathParams`, `MatchedRoute`, `Route`, `AsyncRoute`, `Router`, `AsyncRouter`, static/dynamic path precedence, `ANY` and multi-method route registration, 404 dispatch, unsupported-method `405` adapter responses, custom not-found handlers, fallible sync/async route handlers with router-level and typed error handlers, `POWERTOOLS_DEV` default internal-error detail rendering, router composition with path prefixes, router-level and route-specific request/response middleware, matched-route metadata for handlers and response middleware, request-scoped and shared typed extensions, `CorsConfig`, origin-aware preflight responses, wildcard/additional origins, non-wildcard credential headers, AWS default request headers, routed/404/error CORS headers, optional metrics middleware, optional trace record middleware, optional AppSync GraphQL scalar helpers, optional router-level and route-specific validation hooks, optional gzip/deflate compression middleware, optional sync/async AppSync direct and batch resolver routing and composition, optional AppSync Events routing and composition, optional Bedrock Agent OpenAPI adapter, optional sync/async Bedrock Agent function-details resolver, optional ALB, Lambda Function URL, and VPC Lattice adapters, optional API Gateway REST API v1 / HTTP API v2 / WebSocket API adapters, and initial docs/snippet | Additional resolver families |
| Testing | `LambdaContextStub`, `HandlerHarness`, parameter provider stub re-export, optional feature flag store stub, optional idempotency store stub, optional S3 object client stub, text/bytes fixture readers, and JSON fixture decoder | Additional fake AWS provider stubs |

## Next Durable Work

The next durable work should turn the landed primitives into Lambda-facing utilities:

1. Expand parser envelopes and fixtures using `aws_lambda_events` as the default event model source.
2. Add event-handler adapters for additional resolver families and document the current HTTP, WebSocket, ALB, Lambda
...
| --- | --- | --- |
| `victors-lambdas` | Primary user-facing crate | Depends on support crates through optional dependencies and re-exports enabled utilities |
| `victors-lambdas-core` | Shared foundations | Keep small: config, env, cold start, metadata, and other genuine cross-crate foundations |
| `victors-lambdas-metadata` | Lambda execution metadata | Execution-environment endpoint client, local/dev empty result behavior, process-level cache, status-aware errors, initial docs/snippet, and umbrella exports exist; next work is deployment examples |
| `victors-lambdas-logger` | Structured logs | JSON renderer, `POWERTOOLS_DEV` pretty rendering, sampling, correlation IDs, Lambda context fields, key redaction, custom formatter/redaction hooks, bounded request-keyed log buffering, optional `tracing` subscriber layer, and initial docs/snippet exist; next work is broader handler examples |
| `victors-lambdas-metrics` | CloudWatch EMF metrics | Renderer, flush API, high-resolution metrics, default dimensions, explicit timestamps, overflow flush helpers, async capture helpers, `POWERTOOLS_DEV` disable behavior, and initial docs/snippet exist; next work is broader handler examples |
| `victors-lambdas-tracer` | Tracing facade | Segment records, X-Ray header propagation helpers, optional X-Ray-compatible subsegment document rendering, optional X-Ray daemon UDP transport, optional `tracing` span conversion, optional OpenTelemetry span builder/attribute export, and a buildable OpenTelemetry SDK/stdout plus OTLP exporter snippet exist; next work is vendor-specific OpenTelemetry deployment examples |
| `victors-lambdas-parameters` | Parameter retrieval | Sync and async traits, cache facades, async provider/retrieval errors, in-memory provider, optional SSM single-parameter, by-name, and path providers plus set operations, optional Secrets Manager, AppConfig, and DynamoDB providers, force-fetch support, JSON/base64/auto transforms, initial docs/snippet, and buildable AWS provider snippet exist; additional deployment examples are next |
| `victors-lambdas-jmespath` | JMESPath extraction | Compiled reusable expressions, one-off search helpers, typed result decoding, Powertools JSON/base64/base64-gzip functions, common Lambda event envelope constants, optional idempotency key extraction and payload validation integration, optional validation envelope integration, initial docs/snippet, and umbrella exports exist; next work is data-masking integration examples |
| `victors-lambdas-data-masking` | Data masking | Whole-value erasure, JSON Pointer, dot-path, and common JSONPath-style field masking, fixed/dynamic/custom/regex masking strategies, per-field masking rules, provider encryption/decryption, optional direct AWS KMS provider, configurable missing-field behavior, initial docs/snippet, and umbrella exports exist; next work is AWS Encryption SDK envelope encryption/cached data keys and advanced JSONPath operators beyond the supported selector subset |
| `victors-lambdas-kafka` | Kafka consumer records | Lambda Kafka event flattening, primitive and `JSON` base64 key/value deserializers, schema-aware key/value decoder configuration with Event Source Mapping metadata capture, optional Avro and Protobuf base64 decode helpers, decoded headers, original field/header preservation, initial docs/snippet, and umbrella exports exist; next work is richer schema examples |
| `victors-lambdas-streaming` | Seekable streaming | `RangeSource`, `SeekableStream`, `AsyncRangeSource`, `AsyncSeekableStream`, local byte range source, `S3Object` convenience reader, S3 object range source abstraction, optional AWS SDK-backed S3 client adapter, gzip decoder, CSV reader, and ZIP archive transforms, initial docs/snippet, and umbrella exports exist; next work is larger streaming examples |
| `victors-lambdas-parser` | Event parsing | serde JSON facade plus Transfer Family authorizer event/response, AppSync Events, Bedrock Agent OpenAPI, Bedrock Agent function-details, CloudWatch dashboard custom widget, DynamoDB stream on-failure destination, S3 EventBridge notification, S3 event notification with Intelligent-Tiering support, IoT Core registry, Cognito migrate-user, and Cognito custom sender event models, API Gateway Lambda authorizer aliases, API Gateway WebSocket lifecycle aliases, AppSync Lambda authorizer aliases, Cognito User Pool trigger aliases, common Powertools parser event and record model aliases, including Kinesis-delivered DynamoDB stream record aliases, CloudFormation custom resource request/response aliases, API Gateway REST API, HTTP API, and WebSocket API, AppSync direct resolver/batch aliases and arguments/source, AppSync Events publish payload, Bedrock Agent OpenAPI input text, ActiveMQ message data, ALB, Lambda Function URL, VPC Lattice, SQS, SNS, SNS-over-SQS, RabbitMQ message data, S3, owned S3 event notification, S3-over-SQS, owned S3-over-SQS notification, S3 Object Lambda, S3 Batch, EventBridge with Scheduler empty-detail compatibility, CloudFormation custom resource properties, Cognito User Pool trigger user attributes, SES, Auto Scaling, AWS Config, CloudWatch Alarms, CodeCommit, CodeDeploy lifecycle hook, CodePipeline job, Connect contact flow, CloudWatch Logs, Kinesis, Kinesis-delivered CloudWatch Logs messages, Kinesis-delivered DynamoDB stream image, Firehose, Firehose-delivered SQS, DynamoDB stream image pairs and image-specific helpers, Kafka `aws_lambda_events` envelopes, Secrets Manager rotation, initial docs/snippet, and JSON fixtures for API Gateway REST/HTTP/WebSocket lifecycle/authorizer, VPC Lattice, AppSync, AppSync authorizer, AppSync batch, AppSync Events, Bedrock, Cognito, EventBridge, EventBridge Scheduler, SQS, ActiveMQ, ALB, Lambda Function URL, SNS, RabbitMQ, S3, S3 Intelligent-Tiering, S3 Object Lambda, S3 Batch, S3-over-SQS, SNS-over-SQS, SES, Auto Scaling, AWS Config, CloudWatch Alarm, CloudWatch custom widget, CodeCommit, CodeDeploy lifecycle hook, CodePipeline job, Connect contact flow, Secrets Manager rotation, CloudFormation request/response/stream/nested-stream events, Transfer Family, Cognito custom and migrate-user, DynamoDB stream on-failure, S3 EventBridge, and IoT registry events exist; broader envelope coverage and fixtures are next |
| `victors-lambdas-batch` | Partial batch responses | Generic sequential/concurrent processing, stream checkpoint helpers, SQS, Kinesis, DynamoDB stream, and Kafka adapters, Kafka partition/offset partial response shape, parser-integrated SQS message body, Kinesis record data, and DynamoDB stream image processing, and initial docs/snippet exist; larger examples are next |
| `victors-lambdas-idempotency` | Deduplication | JSON payload hashing, JSON Pointer and optional JMESPath key extraction, configurable full/subset/no payload validation including optional JMESPath validation selection, sync and async handler workflows, Lambda remaining-time in-progress expiry, replay, records, in-memory store, generic async external TTL cache store, local cache wrapper, optional DynamoDB persistence, initial docs/snippet, and a buildable DynamoDB-backed snippet exist; additional deployment examples are next |
| `victors-lambdas-validation` | Payload validation | Basic validators, inbound/outbound wrappers, optional JSON Schema validation, schema caching, optional JMESPath envelope extraction before schema validation, event-handler validation hooks, and initial docs/snippet exist; next work is richer handler examples |
| `victors-lambdas-feature-flags` | Feature flag evaluation | Typed configuration, sync/async rule evaluation, in-memory and optional AppConfig stores, boolean/JSON-valued flags, enabled-feature listing, cache policies, common comparators, modulo matching, time-window rule actions, and initial docs/snippet exist; next work is richer examples |
| `victors-lambdas-event-handler` | Routing | Dependency-free sync/async routing, multi-method route registration, built-in HTTP errors, unsupported-method `405` adapter responses, custom not-found handlers, fallible sync/async route error handling with typed handlers, `POWERTOOLS_DEV` default internal-error detail rendering, router composition with path prefixes, router-level and route-specific middleware, matched-route metadata, request-scoped and shared typed extensions, origin-aware CORS with wildcard/additional origins, non-wildcard credential headers, and AWS default request headers, optional metrics middleware, optional trace record middleware, optional AppSync GraphQL scalar helpers, optional router-level and route-specific validation hooks, optional compression middleware, optional sync/async AppSync direct and batch resolver routing and composition, optional AppSync Events routing and composition, optional Bedrock Agent OpenAPI adapter, optional sync/async Bedrock Agent function-details resolver, optional ALB, Lambda Function URL, VPC Lattice, API Gateway REST API, HTTP API, and WebSocket API adapters, and initial docs/snippet exist; next work is additional event adapters |
| `victors-lambdas-testing` | Test helpers | Context stubs, handler harnesses, parameter provider stubs, optional feature flag and idempotency store stubs, optional S3 object client stubs, and fixture loaders exist; expand additional fake provider stubs only as real utilities need them |
...
- `idempotency-cache-client-examples`
- `parser-serde`
- `parser-schemars`
...
- [x] Add logger bounded request-keyed log buffering.
...
- [x] Add buildable OpenTelemetry OTLP exporter tracer snippet.
...
- [x] Add buildable AWS-backed parameters provider snippet.
- [x] Add JMESPath extraction utility with Powertools JSON/base64/base64-gzip functions.
- [x] Add first-pass data masking utility with field erasure and masking strategies.
- [x] Add data masking provider encryption/decryption and direct AWS KMS provider.
...
- [x] Add Kafka schema-aware consumer configuration and Event Source Mapping schema metadata capture.
- [x] Add first-pass seekable streaming utility with gzip and CSV transforms.
- [x] Add ZIP archive transform for the streaming utility.
- [x] Add S3 object range source abstraction for streaming.
- [x] Add AWS SDK-backed S3 range source adapter for streaming.
- [x] Add async-native seekable stream facade.
...
- [x] Add API Gateway WebSocket lifecycle aliases and fixtures based on `aws_lambda_events`.
- [x] Add AppSync direct resolver parser argument/source envelopes based on `aws_lambda_events`.
- [x] Add AppSync direct resolver and batch resolver aliases and fixtures based on `aws_lambda_events`.
...
- [x] Add combined DynamoDB stream NewImage/OldImage parser envelope based on `aws_lambda_events`.
...
- [x] Add ActiveMQ and RabbitMQ parser message-data envelopes based on `aws_lambda_events`.
...
- [x] Add EventBridge Scheduler empty-detail parser compatibility and fixture.
...
- [x] Add parser Cognito User Pool trigger fixture.
- [x] Add API Gateway Lambda authorizer aliases and fixtures based on `aws_lambda_events`.
- [x] Add CloudFormation custom resource aliases and request/response fixtures based on `aws_lambda_events`.
- [x] Add common Powertools parser event model aliases based on `aws_lambda_events`.
- [x] Add CloudWatch Alarm and Secrets Manager rotation aliases and fixtures based on `aws_lambda_events`.
- [x] Add Auto Scaling, AWS Config, and CodeCommit aliases and fixtures based on `aws_lambda_events`.
- [x] Add AppSync authorizer, CodeDeploy lifecycle hook, CodePipeline job, and Connect contact flow aliases and fixtures
      based on `aws_lambda_events`.
- [x] Add CloudWatch dashboard custom widget model and fixture.
- [x] Add S3 event notification model and Intelligent-Tiering fixture.
- [x] Add Cognito User Pool trigger aliases based on `aws_lambda_events`.
- [x] Add parser record-level aliases, including Kinesis-delivered DynamoDB records, based on `aws_lambda_events`.
- [x] Add parser custom event family fixtures for Transfer Family, Cognito migrate/custom sender, DynamoDB stream
      on-failure, S3 EventBridge, and IoT Core registry models.
...
- [x] Add generic async external TTL cache store for idempotency persistence.
- [x] Add local cache wrapper for idempotency stores.
- [x] Add optional JMESPath idempotency key extraction.
- [x] Add optional JMESPath idempotency payload validation selection.
- [x] Add buildable DynamoDB-backed idempotency snippet.
...
- [x] Add event-handler AppSync GraphQL scalar helpers.
...
- [x] Add event-handler `POWERTOOLS_DEV` default error detail rendering.
...
- [x] Add event-handler HTTP metrics middleware using route-pattern dimensions.
- [x] Add event-handler HTTP trace record middleware using exporter-neutral trace segments.
...
- [x] Add testing S3 object client stub.
- [x] Add testing feature flag and idempotency store stubs.

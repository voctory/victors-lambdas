...
| Logger | `LoggerConfig`, `LogLevel`, `Logger`, `LogEntry`, `LogValue`, JSON rendering, persistent fields, temporary fields, event rendering toggle, level filtering, stdout emission | Sampling, correlation ID extraction, redaction hooks, Lambda context helpers, `tracing` subscriber integration |
| Metrics | `MetricsConfig`, `Metric`, `MetricUnit`, `MetadataValue`, EMF JSON renderer, dimensions, metadata, name/value validation, service dimension, cold-start metric, CloudWatch limits | Explicit flush API, high-resolution metrics, default dimension helpers, async handler ergonomics |
...
1. Harden logger and metrics: sampling, correlation IDs, redaction hooks, metrics flush ergonomics, high-resolution
   metrics, and buildable docs/snippets.
...
| `aws-lambda-powertools-logger` | Structured logs | First JSON renderer exists; next work should avoid forcing one subscriber setup |
| `aws-lambda-powertools-metrics` | CloudWatch EMF metrics | Renderer exists; next work is flush ergonomics and feature completeness |
...
- [ ] Add logger sampling, redaction hooks, correlation IDs, and Lambda context helpers.
- [ ] Add metrics flush ergonomics, high-resolution metrics, and default dimension helpers.

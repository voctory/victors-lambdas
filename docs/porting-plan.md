...
| `aws-lambda-powertools-idempotency` | Deduplication | JSON payload hashing, JSON Pointer and optional JMESPath key extraction, configurable full/subset/no payload validation including optional JMESPath validation selection, sync and async handler workflows, Lambda remaining-time in-progress expiry, replay, records, in-memory store, local cache wrapper, optional DynamoDB persistence, initial docs/snippet, and a buildable DynamoDB-backed snippet exist; additional deployment examples are next |

...
- Feature flags: the umbrella crate exposes `logger`, `logger-tracing`, `metrics`, `tracer`, `tracer-opentelemetry`,
  ...
  `streaming`, `streaming-csv`, `streaming-gzip`, `streaming-s3`, `streaming-zip`, `batch`,
...
| Streaming | `RangeSource`, `SeekableStream`, `BytesRangeSource`, `S3RangeSource`/`S3ObjectClient`, optional AWS SDK-backed S3 client adapter, gzip decoder, CSV reader, and ZIP archive transforms, initial docs/snippet, and umbrella exports | Async-native stream facade |
...
| `aws-lambda-powertools-streaming` | Seekable streaming | `RangeSource`, `SeekableStream`, local byte range source, S3 object range source abstraction, optional AWS SDK-backed S3 client adapter, gzip decoder, CSV reader, and ZIP archive transforms, initial docs/snippet, and umbrella exports exist; next work is an async-native stream facade |

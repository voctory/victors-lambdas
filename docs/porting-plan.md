...
| `aws-lambda-powertools-logger` | Structured logs | JSON renderer, `POWERTOOLS_DEV` pretty rendering, sampling, correlation IDs, Lambda context fields, key redaction, custom formatter/redaction hooks, bounded request-keyed log buffering, optional `tracing` subscriber layer, and initial docs/snippet exist; next work is broader handler examples |
| `aws-lambda-powertools-metrics` | CloudWatch EMF metrics | Renderer, flush API, high-resolution metrics, default dimensions, explicit timestamps, overflow flush helpers, async capture helpers, `POWERTOOLS_DEV` disable behavior, and initial docs/snippet exist; next work is broader handler examples |
...
| `aws-lambda-powertools-streaming` | Seekable streaming | `RangeSource`, `SeekableStream`, `AsyncRangeSource`, `AsyncSeekableStream`, local byte range source, `S3Object` convenience reader, S3 object range source abstraction, optional AWS SDK-backed S3 client adapter, gzip decoder, CSV reader, and ZIP archive transforms, initial docs/snippet, and umbrella exports exist; next work is larger streaming examples |
...
| `aws-lambda-powertools-idempotency` | Deduplication | JSON payload hashing, JSON Pointer and optional JMESPath key extraction, configurable full/subset/no payload validation including optional JMESPath validation selection, sync and async handler workflows, Lambda remaining-time in-progress expiry, replay, records, in-memory store, generic async external TTL cache store, local cache wrapper, optional DynamoDB persistence, initial docs/snippet, and a buildable DynamoDB-backed snippet exist; additional deployment examples are next |
| `aws-lambda-powertools-validation` | Payload validation | Basic validators, inbound/outbound wrappers, optional JSON Schema validation, schema caching, optional JMESPath envelope extraction before schema validation, event-handler validation hooks, and initial docs/snippet exist; next work is richer handler examples |
...
| `aws-lambda-powertools-testing` | Test helpers | Context stubs, handler harnesses, parameter provider stubs, optional feature flag and idempotency store stubs, optional S3 object client stubs, and fixture loaders exist; expand additional fake provider stubs only as real utilities need them |

...
- Feature flags: the umbrella crate exposes `logger`, `logger-tracing`, `metrics`, `tracer`, `parameters`,
  `parameters-secrets`, `parameters-ssm`, `parser`, `parser-aws-lambda-events`, `batch`, `batch-aws-lambda-events`,
  `idempotency`, `validation`, `validation-jsonschema`, `event-handler`, `event-handler-compression`,
...
| `aws-lambda-powertools-parameters` | Parameter retrieval | Sync and async traits, cache facades, async provider/retrieval errors, in-memory provider, optional SSM single-parameter provider, optional Secrets Manager provider, force-fetch support, and JSON/base64 transforms exist; remaining AWS providers are next |
| `aws-lambda-powertools-parser` | Event parsing | serde JSON facade plus API Gateway, ALB, Lambda Function URL, SQS, SNS, EventBridge, CloudWatch Logs, Kinesis, and Firehose `aws_lambda_events` envelopes exist; broader envelope coverage and fixtures are next |
...
| `aws-lambda-powertools-event-handler` | Routing | Dependency-free sync/async routing, middleware, CORS, optional compression middleware, optional AppSync direct resolver routing, optional Bedrock Agent adapter, and optional API Gateway adapters exist; next work is additional event adapters and docs |

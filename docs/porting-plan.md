...
| JMESPath | `JmespathExpression`, `search`, `search_as`, `query`, `extract_data_from_envelope`, Powertools `powertools_json`, `powertools_base64`, and `powertools_base64_gzip` functions, common Lambda event envelope constants, optional idempotency key extraction integration, and initial docs/snippet | Data-masking integration examples |
...
| Idempotency | `IdempotencyConfig`, `IdempotencyKey`, `Idempotency`, `AsyncIdempotency`, `IdempotencyOutcome`, typed workflow errors, SHA-256 JSON payload hashing, JSON Pointer and optional JMESPath key extraction, sync and async handler wrappers, payload hash validation, Lambda remaining-time in-progress expiry, result replay, sync and async store traits/errors/results, in-memory store, local cache wrapper, optional DynamoDB store, initial docs/snippet, and buildable DynamoDB-backed snippet | Additional deployment examples |
...
| `aws-lambda-powertools-jmespath` | JMESPath extraction | Compiled reusable expressions, one-off search helpers, typed result decoding, Powertools JSON/base64/base64-gzip functions, common Lambda event envelope constants, optional idempotency key extraction integration, initial docs/snippet, and umbrella exports exist; next work is data-masking integration examples |
...
| `aws-lambda-powertools-idempotency` | Deduplication | JSON payload hashing, JSON Pointer and optional JMESPath key extraction, sync and async handler workflows, Lambda remaining-time in-progress expiry, replay, records, in-memory store, local cache wrapper, optional DynamoDB persistence, initial docs/snippet, and a buildable DynamoDB-backed snippet exist; additional deployment examples are next |

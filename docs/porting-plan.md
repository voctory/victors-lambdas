...
| `aws-lambda-powertools-parameters` | Parameter retrieval | Sync and async traits, cache facades, async provider/retrieval errors, in-memory provider, optional SSM single-parameter, by-name, and path providers, optional Secrets Manager provider, force-fetch support, and JSON/base64 transforms exist; remaining AWS providers are next |

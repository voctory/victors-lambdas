...
- Feature flags: the umbrella crate exposes `logger`, `logger-tracing`, `metrics`, `tracer`, `tracer-opentelemetry`,
  ...
  `streaming`, `streaming-csv`, `streaming-gzip`, `streaming-zip`, `batch`, `batch-aws-lambda-events`, `batch-parser`,
...
| Streaming | `RangeSource`, `SeekableStream`, `BytesRangeSource`, `S3RangeSource`/`S3ObjectClient`, optional gzip decoder, CSV reader, and ZIP archive transforms, initial docs/snippet, and umbrella exports | `aws-sdk-s3` client adapter |
...
| `aws-lambda-powertools-streaming` | Seekable streaming | `RangeSource`, `SeekableStream`, local byte range source, S3 object range source abstraction, optional gzip decoder, CSV reader, and ZIP archive transforms, initial docs/snippet, and umbrella exports exist; next work is an `aws-sdk-s3` client adapter |

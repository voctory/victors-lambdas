...
| JMESPath | `JmespathExpression`, `search`, `search_as`, `query`, `extract_data_from_envelope`, Powertools `powertools_json`, `powertools_base64`, and `powertools_base64_gzip` functions, common Lambda event envelope constants, optional idempotency key extraction and payload validation integration, and initial docs/snippet | Data-masking integration examples |
...
| `aws-lambda-powertools-jmespath` | JMESPath extraction | Compiled reusable expressions, one-off search helpers, typed result decoding, Powertools JSON/base64/base64-gzip functions, common Lambda event envelope constants, optional idempotency key extraction and payload validation integration, initial docs/snippet, and umbrella exports exist; next work is data-masking integration examples |

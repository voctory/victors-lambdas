...
1. Expand parameter provider docs and examples. Keep AWS SDK dependencies aligned with the documented MSRV.
2. Expand parser envelopes and fixtures using `aws_lambda_events` as the default event model source.
3. Expand idempotency examples where AWS retry semantics overlap.
4. Add event-handler adapters for additional resolver families and document the current HTTP, WebSocket, ALB, Lambda

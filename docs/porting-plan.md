...
5. Expand idempotency where AWS retry semantics overlap: DynamoDB persistence, conditional writes, and concurrency
   behavior.
6. Add event-handler adapters such as Bedrock Agent.
...
| `aws-lambda-powertools-event-handler` | Routing | Dependency-free sync/async routing, middleware, CORS, optional compression middleware, optional AppSync direct resolver routing, and optional API Gateway adapters exist; next work is additional event adapters |

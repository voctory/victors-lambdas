...
| `aws-lambda-powertools-parser` | Event parsing | serde JSON facade plus Transfer Family authorizer event/response models and API Gateway REST API, HTTP API, and WebSocket API, AppSync direct resolver arguments/source, Bedrock Agent input text, ALB, Lambda Function URL, VPC Lattice, SQS, SNS, SNS-over-SQS, S3, S3-over-SQS, S3 Object Lambda, S3 Batch, EventBridge, CloudFormation custom resource properties, Cognito User Pool trigger user attributes, SES, CloudWatch Logs, Kinesis, Firehose, Firehose-delivered SQS, DynamoDB stream image, and Kafka `aws_lambda_events` envelopes exist; broader envelope coverage and fixtures are next |

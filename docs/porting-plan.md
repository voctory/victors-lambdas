...
| `aws-lambda-powertools-event-handler` | Routing | Dependency-free sync/async routing, router composition with path prefixes, middleware, CORS, optional validation hooks, optional compression middleware, optional sync/async AppSync direct and batch resolver routing, optional AppSync Events routing, optional Bedrock Agent OpenAPI adapter, optional sync/async Bedrock Agent function-details resolver, optional ALB, Lambda Function URL, and VPC Lattice adapters, and optional API Gateway REST API, HTTP API, and WebSocket API adapters exist; next work is additional event adapters and docs |

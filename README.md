...
- serde JSON parsing facade with structured parse errors and optional `aws_lambda_events` API Gateway
  ...
  Firehose-delivered SQS, DynamoDB stream image pairs, and Kafka envelopes, plus Transfer Family authorizer
  event/response, AppSync Events, Bedrock Agent OpenAPI and function-details event models/input text, DynamoDB stream
  on-failure destination, S3 EventBridge notification, S3 event notification with Intelligent-Tiering support, IoT Core
  registry, Cognito migrate-user, and Cognito custom sender event models
...
- idempotency keys, payload hashing, JSON Pointer and optional JMESPath key extraction, sync and async handler
  workflows, replay behavior, Lambda remaining-time in-progress expiry, sync and async store traits, an in-memory
  store, an optional local cache wrapper, an optional DynamoDB store, and buildable local/AWS snippets
...
- testing helper surfaces for Lambda context stubs, handler harnesses, parameter provider stubs, optional S3 object
  client stubs, and fixture loading
